...
const validationSchema = Yup.object().shape({
  home_title: Yup.string().required('Title is required'),
  home_description: Yup.string().required('Description is required'),
})
...
const Index: FC<any> = () => {
  ...
  const initialValues = {
    home_title: detailConfig?.home_title || '',
    home_description: detailConfig?.home_description || '',
  }
  ...
  const formik = useFormik({
    ...
    onSubmit: async (val: any) => {
      ...
      const params: FormValues = {
        home_title: val?.home_title,
        home_description: val?.home_description,
      }
      ...
      <form action='' onSubmit={formik.handleSubmit}>
        <div className='bg-white shadow-xs radius-15'>
          <div className='border-bottom border-gray-300 py-10px px-20px m-0 d-flex align-items-center'>
            ...
            <div className='fw-bold fs-14px'>Home Settings</div>
          </div>
