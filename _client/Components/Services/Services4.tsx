...
const Services4 = () => {
  const productCategoryQuery: any = useQuery({
    initialData: { data: [] },
    queryKey: ['getProductCategory', { page: 1, limit: 4 }],
    queryFn: () => getProductCategory({ page: 1, limit: 4 }),
    select: ({ data }: any) => {
          ...
          {productCategory.map((item, key: number) => (
            <div
              ...
              <div className='service4-box bg-white'>
                ...
                <div className='heading4'>
                  <h4>
                    <Link href={item?.id ? `product/category/${item?.id}` : '#'}>{item?.name}</Link>
                  </h4>
                  ...
                  <div className='space16'></div>
                  <Link
                    href={item?.id ? `product/category/${item?.id}` : '#'}
                    className='learn-btn'>
                    Read More
                    <span>
        ...
        <div className='row'>
          <div className='col-lg-12 text-center'>
            <div className='' data-aos='zoom-in-up' data-aos-duration='700'>
              <Link className='theme-btn5' href='/product/category'>
                More Services
                <span>
